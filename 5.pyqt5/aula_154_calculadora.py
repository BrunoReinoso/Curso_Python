...
class Calculadora(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.grid.addWidget(self.display, 0, 0, 1, 5)  #(objeto, posição linha, posição coluna, tamanho linha, tamanho coluna)
        ...
        self.add_btn(
            QPushButton('C'), 1, 4, 1, 1, lambda: self.display.setText(''), '* {background: red; color: #000;}'
        )
        ...
        self.add_btn(
            QPushButton('<-'), 2, 4, 1, 1, lambda: self.display.setText(self.display.text()[:-1]), '* {background: #d5580d; color: #000;}'
        )
        ...
        self.add_btn(
            QPushButton('='), 4, 4, 1, 1, self.eval_igual, '* {background: green; color: #000;}'
        )
